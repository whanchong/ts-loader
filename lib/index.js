...
function dataAttribute2Boolean(value) {
  return (typeof value === 'undefined' || value.toLowerCase() === 'true') ? true : false;
}
...
function getConfig() {
  ...
  var dataset = Object.keys(currentlyLoadedScript.dataset).reduce(function(values, key){
    values[key] = currentlyLoadedScript.dataset[key];
    return values;
  }, {});
  dataset.useLocalCache = dataAttribute2Boolean(dataset.useLocalCache);
  dataset.performHash = dataAttribute2Boolean(dataset.performHash);
  dataset.rewriteSourcemaps = dataAttribute2Boolean(dataset.rewriteSourcemaps);
  dataset.ignoreAppSettings = dataAttribute2Boolean(dataset.ignoreAppSettings);
  return dataset;
}
