...
var hasSupportedBrowserFeatures = require('./has-supported-browser-features.js');
...
function unsupportedBrowserLoad(config) {
...
function normalLoad(manifest, fileCache, config) {
...
loader.load = function (runtimeConfig) {
  ...
  moreConfig().then(function (moreConfig) {
    ...
    if (hasSupportedBrowserFeatures) {
      return normalLoad(manifest, fileCache, config);
    } else {
      return unsupportedBrowserLoad(config);
    }
  }).then(function () {
    loader.emit('loaded');
  }).catch(function (e) {
